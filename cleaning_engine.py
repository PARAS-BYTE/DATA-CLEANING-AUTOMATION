...
from typing import TypedDict, Annotated, Optional, Dict, Any, Generator
...
# Default settings matching something.py
...
def summarize_csv(
    ...
    add("Columns: " + ", ".join(str(c) for c in df.columns))
    ...
    for col in df.columns:
        ...
        unique_pct = (unique / len(df) * 100 if len(df) else 0)
        dtype = str(s.dtype)
        profile_rows.append(
            {
                "column": str(col),
    ...
    add("\n=== DUPLICATES ===")
    add(f"Duplicate rows: {duplicate_count:,} ({duplicate_pct:.2f}%)")

    # Constant Columns
    constant_cols = [str(c) for c in df.columns if df[c].nunique(dropna=False) <= 1]
    add("\n=== CONSTANT COLUMNS ===")
    add(", ".join(constant_cols) if constant_cols else "None")
    ...
    if num_df.empty:
        ...
        for col in num_df.columns:
            ...
            outlier_pct = (outliers / len(df) * 100 if len(df) else 0)
        ...
        add(numeric_summary.to_string(index=False))
        ...
        for i, col1 in enumerate(corr.columns):
            for j, col2 in enumerate(corr.columns):
                ...
                if pd.isna(value):
                    continue
                if abs(value) >= 0.70:
                    pairs.append((abs(value), str(col1), str(col2), float(value)))
        pairs.sort(reverse=True)
        if pairs:
            for _, col1, col2, value in pairs[:max_correlation_pairs]:
                add(f"{col1} <-> {col2}: {value:.3f}")
        else:
    ...
    if cat_df.empty:
        ...
        for col in cat_df.columns:
            ...
            unique_ratio = (unique_count / len(df) if len(df) else 0)
            ...
            add(f"\nColumn: {col}")
            add(f"  Unique: {unique_count:,} ({unique_ratio * 100:.2f}% of rows)")
            add(f"  Missing: {missing_count:,} ({missing_pct:.2f}%)")
            add(f"  High cardinality: {'YES' if unique_ratio > 0.50 else 'NO'}")
            add(f"  Avg string length: {avg_len:.1f}")
            add(f"  Max string length: {max_len:,}")

            top_values = non_null.value_counts().head(top_n_categories)
            if not top_values.empty:
                add("  Most frequent values:")
                for value, count in top_values.items():
                    add(f"    {safe_str(value)} ({count:,})")
    ...
    for col in df.columns:
        ...
        if type_count > 1:
            mixed_cols.append(f"{col} ({type_count} Python types)")
    add("\n".join(mixed_cols) if mixed_cols else "None detected.")
    ...
    for col in cat_df.columns:
        ...
        if ratio >= 0.90:
            suspicious_numeric.append(f"{col}: {ratio * 100:.1f}% numeric-convertible")
    add("\n".join(suspicious_numeric) if suspicious_numeric else "None detected.")
    ...
    for col in cat_df.columns:
        ...
        if len(s) == 0:
            continue
        try:
            converted = pd.to_datetime(s.astype(str), errors="coerce")
            ratio = converted.notna().mean()
            if ratio >= 0.90:
                suspicious_dates.append(f"{col}: {ratio * 100:.1f}% date-convertible")
        except Exception:
            pass
    add("\n".join(suspicious_dates) if suspicious_dates else "None detected.")

    # Representative sample
    add("\n=== REPRESENTATIVE DATA SAMPLE ===")
    if len(df) > 0:
        sample_size = min(max_examples, len(df))
        sample = df.sample(n=sample_size, random_state=42).copy()
        for col in sample.columns:
            sample[col] = sample[col].map(lambda x: safe_str(x))
    ...
    if len(result) > max_output_chars:
        result = (
            ...
            + "\n\n"
            + f"[PROFILE TRUNCATED: output exceeded {max_output_chars:,} characters]"
        )
...
# ============================================================
# 2. PROMPTS (PRESERVED FROM something.py)
# ============================================================
...
def parse_cleaning_required(text: str) -> Optional[bool]:
    match = re.search(r"CLEANING_REQUIRED\s*:\s*(YES|NO)", text, re.IGNORECASE)
    if not match:
        return None
...
def parse_validation_status(text: str) -> Optional[str]:
    match = re.search(r"VALIDATION_STATUS\s*:\s*(PASS|FAIL|BLOCKED)", text, re.IGNORECASE)
    if not match:
        return None
...
def replace_input_path(code: str, dataset_path: str) -> str:
    ...
    for old in replacements:
        code = code.replace(old, f'pd.read_csv(r"{escaped_path}")')
    return code
...
def build_cleaning_graph(
    ...
    """
    Constructs and compiles the StateGraph workflow with customized LLM credentials.
    """
    llm = ChatGroq(
        model=model_name,
        temperature=0,
...
"""
        ...
        if decision is None:
            # Fallback if strict header was omitted
            decision = True if "cleaning" in report.lower() else False
    ...
    def route_after_understanding(state: AgentState):
        if state.get("cleaning_required", True):
    ...
    def cleaning_agent(state: AgentState):
        ...
        history_text = "\n\n".join(history[-5:]) if history else "No previous cleaning history."

        prompt_text = f"""CURRENT DATASET PATH:
...
- Save the final result to {output_filename}.
"""
    ...
    def execute_cleaning(state: AgentState):
        ...
        code_to_run = replace_input_path(code, current_path)
        ...
        try:
            ...
            if not output_path.exists():
                raise RuntimeError(
                    f"Cleaning code executed without an exception, but {output_filename} was not created."
                )

            cleaned_df = pd.read_csv(output_path)
            if cleaned_df.empty:
                raise RuntimeError("Cleaning produced an empty dataset.")

            history_entry = (
                f"Iteration {state.get('iteration', 0) + 1}: "
                f"Cleaning code executed successfully. Output shape: {cleaned_df.shape}."
            )
            ...
            return {
                ...
                "current_dataset_path": str(output_path.resolve()),
                "iteration": state.get("iteration", 0) + 1,
                "cleaning_history": (state.get("cleaning_history", []) + [history_entry]),
                "messages": [HumanMessage(content=history_entry)],
            }

        except Exception as exc:
            error_text = f"Cleaning execution failed: {type(exc).__name__}: {exc}"
            history_entry = f"Iteration {state.get('iteration', 0) + 1}: {error_text}"
            return {
                "execution_success": False,
                "execution_error": error_text,
                "iteration": state.get("iteration", 0) + 1,
                "cleaning_history": (state.get("cleaning_history", []) + [history_entry]),
                "messages": [HumanMessage(content=error_text)],
            }
    ...
    def profile_cleaned(state: AgentState):
        current_path = state["current_dataset_path"]
        summary = summarize_csv(current_path)
    ...
    def validation_agent(state: AgentState):
        ...
        prompt_text = f"""ORIGINAL DATASET SUMMARY:
{state["original_summary"]}
...
"""
        ...
        report = response.content
        status = parse_validation_status(report)

        if status is None:
            # Fallback heuristic
            if "PASS" in report:
                status = "PASS"
            elif "BLOCKED" in report:
                status = "BLOCKED"
            else:
                status = "FAIL"
    ...
    def route_after_validation(state: AgentState):
        if state.get("blocked", False) or state.get("validation_passed", False):
            return "finalize"
        return "cleaning_agent"
    ...
    def finalize(state: AgentState):
        ...
        if state.get("blocked", False):
            final_message = (
                "WORKFLOW BLOCKED.\n\n"
                "The validator determined that the dataset cannot be safely cleaned automatically.\n\n"
            ...
            final_message = (
                ...
                f"Final cleaned dataset: {current_path}\n\n"
                f"Cleaning iterations: {state.get('iteration', 0)}\n\n"
                f"Final validation:\n{state.get('validation_report', '')}"
            )
        else:
            final_message = (
                "WORKFLOW COMPLETED.\n\n"
                "The understanding agent determined that cleaning was not required.\n\n"
                f"Dataset: {current_path}\n\n"
                f"Analysis:\n{state.get('understanding_report', '')}"
            )
    ...
    builder.add_node("profile_original", profile_original)
    builder.add_node("understanding_agent", understanding_agent)
    builder.add_node("cleaning_agent", cleaning_agent)
    builder.add_node("execute_cleaning", execute_cleaning)
    builder.add_node("profile_cleaned", profile_cleaned)
    builder.add_node("validation_agent", validation_agent)
    builder.add_node("finalize", finalize)

    builder.add_edge(START, "profile_original")
    builder.add_edge("profile_original", "understanding_agent")
    builder.add_conditional_edges(
        "understanding_agent",
        route_after_understanding,
        {"cleaning_agent": "cleaning_agent", "finalize": "finalize"},
    )
    builder.add_edge("cleaning_agent", "execute_cleaning")
    builder.add_conditional_edges(
        "execute_cleaning",
        route_after_execution,
        {"profile_cleaned": "profile_cleaned", "cleaning_agent": "cleaning_agent"},
    )
    builder.add_edge("profile_cleaned", "validation_agent")
    builder.add_conditional_edges(
        "validation_agent",
        route_after_validation,
        {"finalize": "finalize", "cleaning_agent": "cleaning_agent"},
    )
...
def stream_data_cleaning(
    ...
    """
    Generator yielding real-time updates for each agent node execution.
    """
    ...
    state: AgentState = {
        "messages": [],
        "original_dataset_path": resolved_path,
        "current_dataset_path": resolved_path,
    ...
    for step_dict in graph.stream(state, config=config, stream_mode="updates"):
        for node_name, node_output in step_dict.items():
